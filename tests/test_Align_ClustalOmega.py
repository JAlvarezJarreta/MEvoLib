...
class AlignTestCase ( unittest.TestCase ) :
    ...
    def standard_test ( self, informat, outformat, params ) :
        ...
        align.get_alignment(clustalo_exe, infile, informat, args=params,
                            outfile=outfile, outfile_format=outformat)
        # Check the output
        self.assertTrue(os.path.isfile(outfile))
        out_align = SeqIO.to_dict(SeqIO.parse(outfile, outformat))
        prevfile = '{}/f001.clustalo_{}.aln'.format(outformat.capitalize(),
                                                    params)
...
class ClustalOmegaTestCase ( AlignTestCase ) :
    ...
    def test_simple_alignment ( self ) :
        ...
        """
        for keyword in viewkeys(align.get_keywords(clustalo_exe)) :
