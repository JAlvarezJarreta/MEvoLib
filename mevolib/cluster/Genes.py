...
def _normalization(
    ...
    record_seq = "".join((x for i, x in enumerate(alignment[1]) if alignment[0][i] != "-"))
...
def map_seqs(
    ...
    alignment_bin: Optional[str] = None,
    ...
    * Reference sequence's file must be in GENBANK format.
    ...
    for record in record_list:
        ...
        if len(record.features) <= 1:
            ...
            if ref_seq:
                record.seq, record.features = _normalization(record, refseq_record, alignment_bin)
            else:
                unprocessable.append(record)
                continue
        record_features = (feat for feat in record.features[1:] if feat.type in gene_dict)
        for feature in record_features:
            ...
            for qualifier_key in iter(_FEAT_QUAL_DICT[feature.type]):
                if qualifier_key in feature.qualifiers:
                    record_qualifiers.update((_string_filter(x) for x in feature.qualifiers[qualifier_key]))
            ...
            # object
            qualifier_id = ":".join(sorted(record_qualifiers, key=lambda item: (len(item), item)))
            feature_record = SeqRecord(
                feature.extract(record).seq, id=record.id, name=record.id, description=qualifier_id
            )
            ...
            for qualifier_key in qualifiers_to_merge:
                if qualifier_key != qualifier_id:
                    gene_dict[feature.type][qualifier_id].extend(gene_dict[feature.type][qualifier_key])
    ...
    threshold = math.ceil((num_seqs * coef) / ((num_seqs - 1.0) * math.pow(e_value, 2) + coef))
                        ...
                        if pair in term_dict[feat_key]:
                            ...
                            if sampling_size < threshold:
                                ...
                                for i in range(0, sampling_size // 6 + 1):
                                    text += "\t\t{}\n".format(" ".join(seq_list[i * 6 : (i + 1) * 6]))
