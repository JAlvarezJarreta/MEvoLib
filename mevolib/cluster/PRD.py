    ...
    * The tree file must be in NEWICK format.
    ...
    try:
        ...
        if len(sets_decomp) == 1:
            raise IOError(
                "DCM can't obtain a decomposition with the given " '"subset_size" and "overlapping" values'
            )
...
def map_seqs(record_list, tree_file, file_format, subset_size, overlapping, binary="dcm"):
    """
    ...
    * The tree file format must be supported by Bio.Phylo.
    ...
    if subset_size < (4 * overlapping):
        raise ValueError(
            "The maximum subset size must be greater than or " "equal to 4 times the overlapping value"
        )
    ...
    set_list, further_decomp = _prd_decomposition(infile_path, subset_size, str(overlapping), binary)
